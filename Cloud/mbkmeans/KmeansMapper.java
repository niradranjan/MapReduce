...
public class KmeansMapper extends CollectiveMapper<String, String, Object, Object> {
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		...
		miniBatchSize = configuration.getInt(KMeansConstants.BATCH_SIZE, 10);
	 ...
	 private void computation(Table<DoubleArray> cenTable, Table<DoubleArray> previousCenTable,ArrayList<DoubleArray> dataPoints){
	  ...
	  private void runKmeans(List<String> fileNames, Configuration conf, Context context) throws IOException {
		  ...
		  for(int iter=0; iter < iteration; iter++){
			  ...
			  computation(cenTable, previousCenTable, batchPoints);
	...
	//Generating Data Points for Mini Batch
	private ArrayList<DoubleArray> generateBatch(ArrayList<DoubleArray> dataPoints) {
		ArrayList<DoubleArray> batch = new ArrayList<>();
		Random rand = new Random();
		int dataPoint = 0;

		//adding "batchSize" random points from the dataPoints
		for(int i = 0; i < miniBatchSize; i++) {
			dataPoint = rand.nextInt(dataPoints.size());
			batch.add(dataPoints.get(dataPoint));
		}
		return batch;
	}
