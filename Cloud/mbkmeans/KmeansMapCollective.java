...
public class KmeansMapCollective  extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		if (args.length < 7) {
			System.err.println("Usage: MiniBatchKmeansMapCollective <numOfDataPoints> <num of Centroids> "
					+ "<size of vector> <batch size> <number of map tasks> <number of iteration> <workDir> <localDir> <communication operation>\n"
		...
		System.out.println("Mini Batch Kmeans Completed");
		return 0;
	}
	void launch(int numOfDataPoints, int numCentroids, int sizeOfVector, int sizeOfBatch, int numMapTasks, int numIteration, String workDir, String localDir, String operation)
		...
		runKMeans(numOfDataPoints,numCentroids, sizeOfVector, sizeOfBatch, numIteration,
	...
	private void runKMeans(int numOfDataPoints, int numCentroids, int vectorSize, int sizeOfBatch, int numIterations,
		...
		do {
			...
			Job kmeansJob = configureKMeansJob(numOfDataPoints,	numCentroids, vectorSize, sizeOfBatch, numMapTasks,
	...
	private Job configureKMeansJob(int numOfDataPoints, int numCentroids, int vectorSize, int sizeOfBatch,
		...
		jobConfig.setInt(KMeansConstants.BATCH_SIZE,sizeOfBatch);
		return job;
	}
